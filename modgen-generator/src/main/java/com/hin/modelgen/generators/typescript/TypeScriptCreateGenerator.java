 ...
 *
 */
public class TypeScriptCreateGenerator extends AbstractGenerator {

  private String componentHtlmTemplate = "typescript/create/component.html.ftl";
  private String componentTsTemplate = "typescript/create/component.ts.ftl";
  private String indexTemplate = "typescript/create/index.ts.ftl";
  ...
  private static List<String> JSR_303_349_VALIDATIONS = Arrays.asList("NotNull", "Min", "Max", "Size",
          "Pattern", "AssertTrue", "AssertFalse", "Digits", "Past", "Future", "DecimalMin", "DecimalMax", "Null");

  public TypeScriptCreateGenerator(String outputDir, String packageName, String entityPackage, String dtoSuffix) {
    super(outputDir);
  ...
  protected void generateFiles(Map<String, Object> input) {
    ...
    getMetaModels().values().forEach(m -> {
      if (m.getType() == ClassifierType.CLASS) {
        ...
        String outputDir = getOutputDir() + "/" + m.getName().toLowerCase() + "s/" + m.getName().toLowerCase() + "-create";
        Utils.checkDirectoryOrCreate(outputDir);

        String fileName = m.getName().toLowerCase() + "-create.component.html";
  ...
  protected void setNameAndVersion() {
    setName("TypeScript Component create");
    setVersion("0.0.1");
  }
